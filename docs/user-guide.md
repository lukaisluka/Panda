...
首次打开落在会话屏（没有 agent 时是空态）。想先看界面长什么样，打开 **demo 回放**：网页版地址栏加 `#/demo`（即 <https://lukaisluka.github.io/Panda/#/demo>；开发服务器同理），Panda 会自动播放一段脚本化的 agent 会话（重构 auth 校验的故事），不需要任何外部服务。回放和真实连接走的是完全相同的内部路径，所以它是了解界面行为的样板：

- 发消息试试——回放驱动会给出脚本化的后续回应
- 遇到权限卡片时点 Allow / Reject，观察 agent 的不同走向
- 想重看一遍，离开再回到 `#/demo` 即可从头回放

想校准长会话下的滚动表现，用长场景回放：`?demo=long` 加路由（即 <https://lukaisluka.github.io/Panda/?demo=long#/demo>，80 轮会话）。
...
- **思考块**（agent 的推理过程，可折叠）
- **工具调用卡片**：标题、状态（pending → in_progress → completed）、涉及文件、文本结果或**代码 diff**（Shiki 语法高亮 + 词级变更高亮）。展开卡片的详情区按工具类型组织：参数不再裸倒 JSON（执行命令显示为终端一行、写入内容显示为代码预览、编辑只留摘要——原文参数由 diff 覆盖），完整参数收在「原始 JSON」折叠里；read/search 的文本结果按文件原样渲染（不过 markdown）；超长内容（整份文件、大 diff）限高渐隐，点「展开全部」恢复；大 diff 还会把长段未变更行折为「⋯ N unchanged lines」占位行（变更段两侧保留 3 行上下文），点占位行展开该段；diff 表头可一键复制为标准 unified 补丁（`git apply` 可用）；agent 附带原始输出对象时折叠为「Output」，已有的渲染结果优先展示。挂起中的卡展开后等待文案区分语义：等审批的显示「等待批准后执行…」，真正在跑的显示「等待输出…」
- **计划卡**：agent 的任务清单及各项状态推进
- **权限卡片**：agent 请求敏感操作批准时出现，点击选项即回复；期间状态栏显示「等待你的批准」，输入框停用。你亲手答过的卡会留下一行折叠记录（已批准/已拒绝 · 选项），策略拒答与会话记忆代答也各有标注卡。注意：重连或切换会话经 `session/load` 重放时**只重建消息与工具流**，审批记录不随回放重现（历史在 agent 侧，回放是消息级的）
- **工具卡状态徽标**：「排队中」表示工具还没轮到执行——最常见的情形是同批工具里另一个触发了审批，整批一起挂起（审批按批恢复），徽标悬停有说明；「等待批准」只出现在真正挂着审批请求的卡上
...
界面文案默认**英文**，可在设置页（`#/settings`）的「语言 / Language」区块切换为**中文**——即时生效，选择按浏览器持久化（`localStorage`），刷新后保持。翻译只覆盖界面自身（按钮、标签、提示、错误消息）；agent 产出的内容（消息、工具结果、diff）与 demo 回放剧本始终按原文显示，不翻译。本指南行文中引用的界面文案（如「排队中」「展开全部」「认证」）以中文界面的显示为准。
