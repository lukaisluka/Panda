...
- **思考块**（agent 的推理过程，可折叠）
- **工具调用卡片**：标题、状态（pending → in_progress → completed）、涉及文件、文本结果或**代码 diff**（Shiki 语法高亮 + 词级变更高亮）。展开卡片的详情区按工具类型组织：参数不再裸倒 JSON（执行命令显示为终端一行、写入内容显示为代码预览、编辑只留摘要——原文参数由 diff 覆盖），完整参数收在「原始 JSON」折叠里；read/search 的文本结果按文件原样渲染（不过 markdown）；超长内容（整份文件、大 diff）限高渐隐，点「展开全部」恢复
